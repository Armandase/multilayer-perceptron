...
class Layer:
    def __init__(self, nodes, input_len ,funcActivation, weights):
        self.nodes = nodes
    ...
    def deltaHiddenLayer(self, above_delta, output, above_weights):
        ...
        for i in range(output.shape[0]):
            delta[i] = output[i] * (1 - output[i]) * np.sum(above_delta * above_weights[i])
        return delta
    
    def update_weights(self, output, delta):
        delta_weights = self.learning_rate * delta * output
